...
if (!require("epidist")) {
  remotes::install_github(
    file.path("epinowcast", "epidist@v0.3.0"),
    dependencies = TRUE
  )
}

if (!require("primarycensored")) {
  remotes::install_github(
    file.path("epinowcast", "primarycensored@v1.2.0"),
    dependencies = TRUE
  )
}


pacman::p_load(tidyverse, glue, epidist, dplyr, modelr, priorsense)
...
# length of extension to upper bound
window_extension <- 3
...
for (symptom_name in symptom_opts) {
  message(glue::glue("running {symptom_name}"))
  ...
  priors <- c(
    # intercepts for mu and sigma
    prior(normal(log(4), 0.5), class = "Intercept"),
    prior(normal(log(0.7), 0.5), class = "Intercept", dpar = "sigma"),
    # priors on fixed effects. Identification of these prior names is a bit challenging
    # so we are having to generalise with the class rather than coeff etc.
    # age mu
    brms::prior(normal(0, 3), class = "b"),
    # age sigma
    brms::prior(normal(0, 3), class = "b", dpar = "sigma")
  )
